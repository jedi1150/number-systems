...
data class NumberSystem(var value: String, var radix: Radix, var isError: Boolean = false)
...
fun NumberSystem.pretty(
    isDigitGroupingEnabled: Boolean = true,
): String = value.split(NumSys.Constants.DELIMITER).joinToString(
    separator = NumSys.Constants.DELIMITER.toString(),
) { part ->
    part
        .reversed()
        .chunked(
            asInternalModel().groupLength(),
        ).joinToString(if (isDigitGroupingEnabled) NumSys.Constants.GROUP_SEPARATOR.toString() else "")
        .reversed()
}
